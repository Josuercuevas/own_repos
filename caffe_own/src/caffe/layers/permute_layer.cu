/*
  GPU implementation for the layer permutation in caffe, to match the memory
...
namespace caffe {
  ...
  // Core kernel to perform data exchange between blobs to be permutated
  ...
  __global__ void PermuteKernel(const int nthreads, Dtype* const bottom_data,
    ...
    Dtype* const top_data){
      ...
      CUDA_KERNEL_LOOP(index, nthreads){
        ...
        // data asignation to top blob data
        if(forward){
          top_data[index] = bottom_data[old_idx];
        }else{
          // data asignation to bottom blob data
          bottom_data[old_idx] = top_data[index];
        }
      }
  }

  // forward pass through the GPU, same idea as in the CPU
  template <typename Dtype>
  void PermuteLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top){
      if(need_permute_){
        ...
        // calling cuda kernel to be used for data processing in GPU
        ...
        /*
        CUDA: check for error after kernel
        execution and exit loudly if there is one.
        */
  ...
  /*
  Backward prop for this layer with support for GPU
  */
  template <typename Dtype>
