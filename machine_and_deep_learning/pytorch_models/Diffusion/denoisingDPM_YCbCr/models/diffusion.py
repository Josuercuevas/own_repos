...
class GaussianDiffusion(nn.Module):
    ...
    def remove_noise(self, x, tstep, yclass, use_ema=True):
        LOGD(f"Removing noise from timestep {tstep}. yclass is {yclass}")
        ...
        if use_ema:
            ...
            # print(f'ema noise ===> min = {torch.min(pred_noise)}, max = {torch.max(pred_noise)}')
        else:
            # model trained without EMA, basically what we get at each iteration
            pred_noise = self.model(x, tstep, yclass)
            # print(f'normal noise ===> min = {torch.min(pred_noise)}, max = {torch.max(pred_noise)}')
        ...
        # print(f"scale factor 1 shape = {scale_factor.size()}, scale factor max {torch.max(scale_factor)}, scale factor min {torch.max(scale_factor)} \npred_noise ===> min = {torch.min(pred_noise)}, max = {torch.max(pred_noise)}")
        
        denoised = x - (scale_factor * pred_noise)
        # print(f"\n\ninput x size {x.size()}, xmin xmax = {torch.min(x)} {torch.max(x)}, \nscale 1 factor size {scale_factor.size()} min max = {torch.min(scale_factor)} {torch.max(scale_factor)} \npred_noise size = {pred_noise.size()} min max = {torch.min(pred_noise)} {torch.max(pred_noise)}\ndenoised size {denoised.size()}, min max = {torch.min(denoised)} {torch.max(denoised)}")
        scale_factor = extract_noise_factor(self.reciprocal_sqrt_alphas, tstep, x.shape)
        # print(f"scale factor 2 shape = {scale_factor.size()}, scale factor max = {torch.max(scale_factor)}, scale factor min = {torch.min(scale_factor)}")
    ...
    def sample(self, batch_size, device, yclass=None, use_ema=True, gen_seq=False):
        ...
        print("Generating some samples/sequences for visualization purposes")
    ...
    def add_noise_x(self, x, tstep, noise):
        ...
        # print(f"noise shape ==> {noisy_x.shape}, min = {torch.min(noisy_x)}, max = {torch.max(noisy_x)}")
        return noisy_x
    ...
    def compute_losses(self, x, tstep, yclass):
        ...
        # print(f"\nnoisy_x shape ==> {noisy_x.shape}, noisy_x min = {torch.min(noisy_x)}, max = {torch.max(noisy_x)}")
        # print(f"\nx shape ==> {x.shape}, x min = {torch.min(x)}, x max = {torch.max(x)}")
    ...
    def forward(self, x, yclass=None):
        ...
        if h != self.img_size[0]:
            print(f"image height ({h}) is different from diffusion params {self.img_size[0]}")
            raise ValueError(f"image height ({h}) is different from diffusion params {self.img_size[0]}")
        if w != self.img_size[1]:
            print(f"image height ({w}) is different from diffusion params {self.img_size[1]}")
