...
def coverageBoundingBox(bbox):
    ...
    if config.gridbox_type == 'GRIDBOX_MIN':
        ...
        exit(1)
    coverage = (np.subtract(center, np.multiply(shrunkSize, 0.5)),
                np.add(center, np.multiply(shrunkSize, 0.5)))
...
def imageRectToGridRect(coverage):
    ...
    # return [max(0, math.floor(1.0*coverage[0]/config.stride)),
    #         max(0, math.floor(1.0*(coverage[1])/config.stride)),
    #         min(grid_x, math.ceil(1.0 * (coverage[2]) / config.stride)),
    #         min(grid_y, math.ceil(1.0 * (coverage[3])/config.stride))]
...
def generateLabel(bboxlist, lbl_classes):
    ...
    label_size = np.zeros((2, int(grid_x), int(grid_y)))
    label_obj_norm = np.zeros((1, int(grid_x), int(grid_y)))
    label_foreground = np.zeros((1,int(grid_x), int(grid_y)))
            ...
            for g_x in range(int(g_coverage[0]), int(g_coverage[2])):
                ...
                if cvgArea > FLT_EPSILON:
                    if g_x < grid_x and g_y < grid_y:
                        ...
                        label_bbox[2, g_x, g_y] = bbox[2] - gridBox[0]
                        #label_bottomRight_y
                        #label_bbox[3+(lbl_classes[idx]*3 + lbl_classes[idx]), g_x, g_y] = bbox[3] - gridBox[1]
                        label_bbox[3, g_x, g_y] = bbox[3] - gridBox[1]
