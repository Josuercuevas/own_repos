...
'''
def Perform_augmentation(database_path, dst_path):
    defined_path = database_path
    ...
    for j in range(len(onlyfolders)):
        ...
        for i in range(len(onlyfiles[j][0])):
            ...
            rotator(im, join(dst_path, onlyfolders[j]), onlyfiles[j][0][i])
            sharpener(im, join(dst_path, onlyfolders[j]), onlyfiles[j][0][i])
            blurrer(im, join(dst_path, onlyfolders[j]), onlyfiles[j][0][i])
            contraster(im, join(dst_path, onlyfolders[j]), onlyfiles[j][0][i])
            Colorer(im, join(dst_path, onlyfolders[j]), onlyfiles[j][0][i])
