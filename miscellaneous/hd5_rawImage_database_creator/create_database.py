'''
    ...
    Creation of the database for training, validating and testing models
...
def perform_data_extraction(class_folder_path, save_bd_at):
    ...
    if AUGMENT_DATA:
        if Perform_augmentation():
            ...
            print("augmentation performed successfully ...")
            sys.exit(0)
    ...
    for j in range(len(onlyfolders)):
        ...
        for i in range(len(onlyfiles[j][0])):
            ...
            print("mode: %s, %s"%(im.mode, onlyfiles[j][0][i]))
            ...
            if im.mode=='L':
                ...
                if channels != 3:
                    ...
                    print("changing number of channels")
                channels = 3
                changed += 1
    ...
    for j in range(len(onlyfolders)):
        ...
        for i in range(len(onlyfiles[j][0])):
            ...
            if changew_count>1 or changeh_count>1 or changed>1:
                print("============> %d, %d, %d"%(changew_count, changeh_count, changed))
                break
            else:
                if change_class:
...
if __name__ == '__main__':
    perform_data_extraction("PATH_TO_SRC_FOLDER_PATH", "PATH_TO_DST_FOLDER_PATH")
